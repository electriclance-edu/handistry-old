...
function enableCam(event) {
  ...
  let loaded_video = document.getElementById("loaded_video");
  // predictVideo(loaded_video);

  navigator.mediaDevices.getUserMedia(constraints).then(function (stream) {
    loaded_video.srcObject = stream;
    loaded_video.addEventListener("loadeddata", predictVideo);
  });
...
function predictVideo(video) {
  ...
  if (runningMode === "IMAGE") {
    runningMode = "VIDEO";
    handLandmarker.setOptions({ runningMode: runningMode });
  }

  let nowInMs = Date.now();
  const results = handLandmarker.detectForVideo(video, nowInMs);
  console.log(nowInMs, results);
